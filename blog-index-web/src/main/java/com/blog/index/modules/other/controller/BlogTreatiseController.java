...
public class BlogTreatiseController {
    ...
    public R getRecommend(){
        //查询列表数据
        List<BlogTreatise> treatiseList=iBlogTreatiseService.selectList(new EntityWrapper<BlogTreatise>().eq("recommend",1).orderBy("create_time",false));
    ...
    public R getReadRanking(@ApiIgnore BaseQuery baseQuery){
        //查询列表数据
        Page page=new Page(baseQuery.getCurrentPage(),baseQuery.getPageSize());
        Page pageList=iBlogTreatiseService.selectPage(page,new EntityWrapper<BlogTreatise>().orderBy("read_num",false));
        if (CollectionUtils.isEmpty(pageList.getRecords())) {
            return R.notFound();
        }
        return R.fillPageData(pageList);
